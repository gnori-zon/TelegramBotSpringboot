...
import gnorizon.SpringTestReportsBot.command.commands.DeleteMeGroupCommand;
...
@DisplayName("Unit-level testing for DeleteMeGroupCommand")
public class DeleteMeGroupCommandTest {
    ...
    public void init(){
        ...
        command = new DeleteMeGroupCommand(sendBotMessageService,modifyDataBaseService);
    }
    ...
    public void shouldProperlyReturnListGroup(){
        ...
        HashMap<String, String> hashMap = new HashMap<>(Map.of("group1", "Владелец", "group2", "Участник"));
    ...
    public void shouldProperlyTextForCaseEmptySelfGroup(){
        ...
        SendMessage message = new SendMessage();
        message.setChatId(String.valueOf(chatId));
        message.setText("Вы не состоите в группах как участник");


        HashMap<String, String> hashMap = new HashMap<>(Map.of("group1", "Владелец"));
        Mockito.when(modifyDataBaseService.getAllGroup(chatId)).thenReturn(hashMap);
        //when
        command.execute(update);
        //then
