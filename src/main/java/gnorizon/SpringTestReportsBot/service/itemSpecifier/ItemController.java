package gnorizon.SpringTestReportsBot.service.itemSpecifier;

import gnorizon.SpringTestReportsBot.model.Reports.Report;
import org.springframework.stereotype.Component;

import static gnorizon.SpringTestReportsBot.service.TelegramBot.FINISH_TYPE;
import static gnorizon.SpringTestReportsBot.service.itemSpecifier.Items.*;

@Component
public class ItemController implements ItemSpecifier{
    private final String LAST_TEXT = "src/main/resources/static/SuccessBot.jpg" + "XSXГотово!";
    private final String EXCEPTION_TEXT = "Кажется вы что-то забыли ввести, попробуйте снова";
    @Override
    public String checkingAndWrite(String message, Report report){
        int regex = report.getName().indexOf('_');
        String typeReport = report.getName().substring(regex+1) ;

        ReportFiller filler = new ReportFiller(report,typeReport);
        char stepNumber = message.charAt(0);
        try {
                if(stepNumber==ITEM_1.step) {
                    filler.fillItem1(message);

                    if (typeReport.equals(FINISH_TYPE)) {
                        return ITEM_2.textForStep;
                    } else {
                        return ITEM_2.textForStep2;
                    }
                }else if(stepNumber==ITEM_2.step) {
                    filler.fillItem2(message);

                    if (typeReport.equals(FINISH_TYPE)) {
                        return ITEM_3.textForStep;
                    } else {
                        return ITEM_3.textForStep2;
                    }
                }else if(stepNumber==ITEM_3.step) {
                    filler.fillItem3(message);

                    if (typeReport.equals(FINISH_TYPE)) {
                        return ITEM_4.textForStep;
                    } else {
                        return ITEM_4.textForStep2;
                    }
                }else if(stepNumber==ITEM_4.step) {
                    filler.fillItem4(message);
                    return ITEM_5.textForStep;
                }else if(stepNumber==ITEM_5.step) {
                    filler.fillItem5(message);

                    return ITEM_6.textForStep;
                }else if(stepNumber==ITEM_6.step) {
                    filler.fillItem6(message);

                    return ITEM_7.textForStep;
                }else if(stepNumber==ITEM_7.step) {
                    filler.fillItem7(message);

                    return ITEM_8.textForStep;
                }else if(stepNumber==ITEM_8.step) {
                    filler.fillItem8(message);

                    return ITEM_9.textForStep;
                }else if(stepNumber==ITEM_9.step) {
                        filler.fillItem9(message);

                        return ITEM_10.textForStep;
                }else if(stepNumber==ITEM_10.step) {
                    filler.fillItem0(message);

                    return LAST_TEXT ;
            }
        }catch (Exception e){
            return EXCEPTION_TEXT ;
        }
        return "oups!";
    }
}
